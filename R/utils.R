...
#'
#' \mjdeqn{\operatorname{B}(x) = \operatorname{exp} \,  \left\lbrace \sum_{i=1}^{k}{log \, \Gamma(x_i)} - log \, \Gamma \left(\sum_{i = 1}^{k}x_i\right)\right\rbrace}{B(x) = exp \{ sum_{i=1}^{k}{log Gamma(x_i)} - log Gamma (sum_{i = 1}^{k}x_i) \}}
#'
...
#'
#' @return If all elements of `x` are greater than zero then `mbeta` returns a (positive) numeric vector of \code{\link[base]{length}} 1. If any element of `x` is equal to zero then `mbeta` returns `inf`.
#'
#' @references
#' \insertAllCited{}
#'
...
#'
#' \mjdeqn{\operatorname{log} \operatorname{B}(x) = \sum_{i=1}^{k}{log \, \Gamma(x_i)} - log \, \Gamma \left(\sum_{i = 1}^{k}x_i\right)}{log B(x) = sum_{i=1}^{k}{log Gamma(x_i)} - log Gamma (sum_{i = 1}^{k}x_i)}
#'
