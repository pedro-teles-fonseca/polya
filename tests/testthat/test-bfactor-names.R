...
testthat::test_that("default transf name", {
  expect_equal(
    round(
      bfactor_multinomial(x = austria_bl1, null.par = theta_benford(1)),
    3),
    c("BF" = 0.001))
})
...
testthat::test_that("level transf name", {
  expect_equal(
    round(
      bfactor_multinomial(x = austria_bl1, null.par = theta_benford(1), transf = "level"),
      3),
    c("BF" = 0.001))
})
...
testthat::test_that("log transf name", {
  expect_equal(
    round(
      bfactor_multinomial(x = austria_bl1, null.par = theta_benford(1), transf = "log"),
    3),
    c("log(BF)" = -7.133))
})
...
testthat::test_that("BF strength of evidence", {
  expect_equal(
      bfactor_interpret(bfactor_multinomial(x = austria_bl1, null.par = theta_benford(1))),
    c("Evidence" = "Negative")
    )
  })
