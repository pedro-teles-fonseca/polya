...
test_that("Bin-Haldane - sucess level not observed test 2", {
  expect_equal(
    round(fbf_binomial(x = c(1,1,1), success = 1, null_par = .2), 2),
    0.27
  )
})
...
test_that("correspondence consistent FBF vs FBF w/MTS", {
  expect_equal(
    fbf_multinomial(austria_bl1, categories = 1:9, null_par = theta_benford(1), hyper_par = 1, b = 9/length(austria_bl1)),
    fbf_multinomial(austria_bl1, categories = 1:9, null_par = theta_benford(1), hyper_par = 1, m = 9, robust = "minimal")
...
test_that("correspondence consistent FBF vs FBF w/MTS", {
  expect_equal(
    fbf_multinomial(austria_bl1, categories = 1:9, null_par = theta_benford(1), hyper_par = rep(1, 9), b = sqrt(9/length(austria_bl1))),
